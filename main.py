...
def start_session(user_id: str, db: Session = next(get_db())):
...
def end_session(session_id: int, db: Session = next(get_db())):
    dao = GameSessionDAO(db)
...
def get_session(session_id: int, db: Session = next(get_db())):
    dao = GameSessionDAO(db)
