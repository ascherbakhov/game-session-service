...
async def start_session(request: StartSessionRequest, db: AsyncSession = Depends(get_db)) -> StartSessionResponse:
...
async def end_session(request: StopSessionRequest, db: AsyncSession = Depends(get_db)) -> EndSessionResponse:
    dao = GameSessionDAO(db)
...
async def get_session(session_id: int, db: AsyncSession = Depends(get_db)) -> GetSessionResponse:
    dao = GameSessionDAO(db)
...
async def heartbeat(session_id: int, db: AsyncSession = Depends(get_db)) -> HeartbeatResponse:
    dao = GameSessionDAO(db)
